...
class VetMessagingController extends ChangeNotifier {
              ...
              callback: (payload) async {
                ...
                if (conversationId == _selectedConversationId) {
                  try {
                    ...
                    final newMessage = await _messagingService.getMessage(
                      messageId,
                    );
                    if (newMessage != null) {
  ...
  Future<void> loadConversations() async {
    ...
    try {
      ...
      for (var i = 0; i < conversations.length; i++) {
        conversations[i]['lastMessage'] = await _messagingService
            .getLastMessage(conversations[i]['id']);

        final participant = await _messagingService.getParticipantInfo(
          conversations[i]['id'],
        );
        if (participant != null) {
  ...
  Future<bool> createConversation({
    ...
    try {
      ...
      if (conversationId != null) {
        selectConversation(conversationId, name ?? 'Unknown');
        await loadConversations();
        return true;
      }
    } catch (e) {}
  ...
  Future<List<Map<String, dynamic>>> loadPetOwners() async {
    try {
